...
export default function SplashCursor({
  ...
  DYE_RESOLUTION = 512, // Reduced from 1440 for much better performance
  CAPTURE_RESOLUTION = 512,
  DENSITY_DISSIPATION = 2.0, // Reduced for smoother, longer-lasting trails
  VELOCITY_DISSIPATION = 1.5, // Reduced for smoother motion
  PRESSURE = 0.1,
  PRESSURE_ITERATIONS = 8, // Reduced from 20 for better performance
  CURL = 2, // Reduced from 3
  SPLAT_RADIUS = 0.15, // Reduced from 0.2
  SPLAT_FORCE = 5000, // Increased for more responsive trails
  ...
  useEffect(() => {
    ...
    function calcDeltaTime() {
      ...
      dt = Math.min(dt, 0.008333); // Allow up to ~120fps for smoother animation
      lastUpdateTime = now;
      return dt;
    }
    ...
    function handleFirstPointerMove(e: PointerEvent) {
      const pointer = pointers[0];
      const posX = scaleByPixelRatio(e.clientX);
      const posY = scaleByPixelRatio(e.clientY);
      const color = generateColor();
      ...
      document.body.removeEventListener('pointermove', handleFirstPointerMove);
    }
    document.body.addEventListener('pointermove', handleFirstPointerMove);
